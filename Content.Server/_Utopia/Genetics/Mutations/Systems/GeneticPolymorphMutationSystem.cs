...
/// <summary>
/// Handles genetic instability-triggered polymorphs. Generic: the target polymorph is configured
/// via <see cref="MutationInstabilityPolymorphComponent.PolymorphId"/> so any entity type can be used.
/// </summary>
public sealed class GeneticPolymorphMutationSystem : EntitySystem
{
    ...
    private void OnStartup(EntityUid oldUid, MutationInstabilityPolymorphComponent component, ComponentStartup args)
    {
        ...
        foreach (var entry in oldGenetics.Mutations)
        {
            ...
            {
                // Skip mutations that would trigger another polymorph, to avoid infinite loops.
